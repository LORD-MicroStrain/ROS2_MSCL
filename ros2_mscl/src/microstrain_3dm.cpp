...
bool Microstrain::configure_node()
{
  ...
  //Raw data file save
  m_raw_file_enable               = this->get_parameter("raw_file_enable").as_bool();
  m_raw_file_include_support_data = this->get_parameter("raw_file_include_support_data").as_bool();
  raw_file_directory              = this->get_parameter("raw_file_directory").as_string();

  
  //Log the MSCL version for debugging purposes
  RCLCPP_INFO(this->get_logger(), "Using MSCL Version: %s", mscl::MSCL_VERSION.str().c_str());
  ...
  try
  {
    ...
    rclcpp::ServiceServer get_basic_status_service;
    rclcpp::ServiceServer get_diagnostic_report_service;
    rclcpp::ServiceServer device_report_service;
    ...
    if(m_inertial_device->features().supportsCommand(mscl::MipTypes::Command::CMD_DEVICE_STATUS))
    {
      ...
      get_basic_status_service      = this->create_service<>("get_basic_status",      &Microstrain::get_basic_status, this);
      get_diagnostic_report_service = this->create_service<>("get_diagnostic_report", &Microstrain::get_diagnostic_report, this);
    }

    if(m_inertial_device->features().supportsCommand(mscl::MipTypes::Command::CMD_GET_DEVICE_INFO))
    {
      device_report_service = this->create_service<>("device_report", &Microstrain::device_report, this);
    }
    */
    ...
    if(m_publish_imu && m_publish_gps_corr)
    {
      ...
      m_gps_corr_pub = this->create_publisher<ros2_mscl_msgs::msg::GPSCorrelationTimestampStamped>("gps_corr", 100);
    }
    ...
    if(m_publish_gnss[GNSS1_ID] && supports_gnss1)
    {
      ...
      if(m_inertial_device->features().supportsCommand(mscl::MipTypes::Command::CMD_EF_AIDING_MEASUREMENT_ENABLE))
      {
        m_gnss_aiding_status_pub[GNSS1_ID]     = this->create_publisher<ros2_mscl_msgs::msg::GNSSAidingStatus>("gnss1/aiding_status", 100);
    ...
    if(m_publish_gnss[GNSS2_ID] && supports_gnss2)
    {
      ...
      if(m_inertial_device->features().supportsCommand(mscl::MipTypes::Command::CMD_EF_AIDING_MEASUREMENT_ENABLE))
      {
        m_gnss_aiding_status_pub[GNSS2_ID]     = this->create_publisher<ros2_mscl_msgs::msg::GNSSAidingStatus>("gnss2/aiding_status", 100);
    ...
    if(m_publish_rtk && supports_rtk)
    {
      ...
      m_rtk_pub =  this->create_publisher<ros2_mscl_msgs::msg::RTKStatus>("rtk/status", 100);
    }
    ...
    if(m_publish_filter && supports_filter)
    {
      ...
      m_filter_status_pub        = this->create_publisher<ros2_mscl_msgs::msg::FilterStatus>("nav/status", 100);
      m_filter_heading_pub       = this->create_publisher<ros2_mscl_msgs::msg::FilterHeading>("nav/heading", 100);
      m_filter_heading_state_pub = this->create_publisher<ros2_mscl_msgs::msg::FilterHeadingState>("nav/heading_state", 100);
    ...
    {
      m_set_tare_orientation_service = this->create_service<ros2_mscl::srv::SetTareOrientation>("set_tare_orientation", std::bind(&Microstrain::set_tare_orientation, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_set_complementary_filter_service = this->create_service<ros2_mscl::srv::SetComplementaryFilter>("set_complementary_filter", std::bind(&Microstrain::set_complementary_filter, this, std::placeholders::_1, std::placeholders::_2));
      m_get_complementary_filter_service = this->create_service<ros2_mscl::srv::GetComplementaryFilter>("get_complementary_filter", std::bind(&Microstrain::get_complementary_filter, this, std::placeholders::_1, std::placeholders::_2));
    }
    
    //IMU sensor2vehicle frame rotation service
    if (m_inertial_device->features().supportsCommand(mscl::MipTypes::Command::CMD_EF_SENS_VEHIC_FRAME_ROTATION_EULER))
    {
      m_set_sensor2vehicle_rotation_service = this->create_service<ros2_mscl::srv::SetSensor2VehicleRotation>("set_sensor2vehicle_rotation", std::bind(&Microstrain::set_sensor2vehicle_rotation, this, std::placeholders::_1, std::placeholders::_2));
      m_get_sensor2vehicle_rotation_service = this->create_service<ros2_mscl::srv::GetSensor2VehicleRotation>("get_sensor2vehicle_rotation", std::bind(&Microstrain::get_sensor2vehicle_rotation, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_set_sensor2vehicle_offset_service = this->create_service<ros2_mscl::srv::SetSensor2VehicleOffset>("set_sensor2vehicle_offset", std::bind(&Microstrain::set_sensor2vehicle_offset, this, std::placeholders::_1, std::placeholders::_2));
      m_get_sensor2vehicle_offset_service = this->create_service<ros2_mscl::srv::GetSensor2VehicleOffset>("get_sensor2vehicle_offset", std::bind(&Microstrain::get_sensor2vehicle_offset, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_get_sensor2vehicle_transformation_service = this->create_service<ros2_mscl::srv::GetSensor2VehicleTransformation>("get_sensor2vehicle_transformation", std::bind(&Microstrain::get_sensor2vehicle_transformation, this, std::placeholders::_1, std::placeholders::_2));
    }

    //IMU Accel bias service
    if (m_inertial_device->features().supportsCommand(mscl::MipTypes::Command::CMD_ACCEL_BIAS))
    {
      m_set_accel_bias_service = this->create_service<ros2_mscl::srv::SetAccelBias>("set_accel_bias", std::bind(&Microstrain::set_accel_bias, this, std::placeholders::_1, std::placeholders::_2));
      m_get_accel_bias_service = this->create_service<ros2_mscl::srv::GetAccelBias>("get_accel_bias", std::bind(&Microstrain::get_accel_bias, this, std::placeholders::_1, std::placeholders::_2));
    }

    //IMU gyro bias service
    if (m_inertial_device->features().supportsCommand(mscl::MipTypes::Command::CMD_GYRO_BIAS))
    {
      m_set_gyro_bias_service = this->create_service<ros2_mscl::srv::SetGyroBias>("set_gyro_bias", std::bind(&Microstrain::set_gyro_bias, this, std::placeholders::_1, std::placeholders::_2));
      m_get_gyro_bias_service = this->create_service<ros2_mscl::srv::GetGyroBias>("get_gyro_bias", std::bind(&Microstrain::get_gyro_bias, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_set_hard_iron_values_service = this->create_service<ros2_mscl::srv::SetHardIronValues>("set_hard_iron_values", std::bind(&Microstrain::set_hard_iron_values, this, std::placeholders::_1, std::placeholders::_2));
      m_get_hard_iron_values_service = this->create_service<ros2_mscl::srv::GetHardIronValues>("get_hard_iron_values", std::bind(&Microstrain::get_hard_iron_values, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_set_coning_sculling_comp_service = this->create_service<ros2_mscl::srv::SetConingScullingComp>("set_coning_sculling_comp", std::bind(&Microstrain::set_coning_sculling_comp, this, std::placeholders::_1, std::placeholders::_2));
      m_get_coning_sculling_comp_service = this->create_service<ros2_mscl::srv::GetConingScullingComp>("get_coning_sculling_comp", std::bind(&Microstrain::get_coning_sculling_comp, this, std::placeholders::_1, std::placeholders::_2));
    }
    
    //Kalman filter reset
    if (m_inertial_device->features().supportsCommand(mscl::MipTypes::Command::CMD_EF_RESET_FILTER))
    {
      m_reset_filter_service = this->create_service<std_srvs::srv::Empty>("reset_kf", std::bind(&Microstrain::reset_filter, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_set_estimation_control_flags_service = this->create_service<ros2_mscl::srv::SetEstimationControlFlags>("set_estimation_control_flags", std::bind(&Microstrain::set_estimation_control_flags, this, std::placeholders::_1, std::placeholders::_2));
      m_get_estimation_control_flags_service = this->create_service<ros2_mscl::srv::GetEstimationControlFlags>("get_estimation_control_flags", std::bind(&Microstrain::get_estimation_control_flags, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_init_filter_euler_service = this->create_service<ros2_mscl::srv::InitFilterEuler>("init_filter_euler", std::bind(&Microstrain::init_filter_euler, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_init_filter_heading_service = this->create_service<ros2_mscl::srv::InitFilterHeading>("init_filter_heading", std::bind(&Microstrain::init_filter_heading, this, std::placeholders::_1, std::placeholders::_2));
    }

    //Kalman filter heading source service
    if(m_inertial_device->features().supportsCommand(mscl::MipTypes::Command::CMD_EF_HEADING_UPDATE_CTRL))
    {
      m_set_heading_source_service = this->create_service<ros2_mscl::srv::SetHeadingSource>("set_heading_source", std::bind(&Microstrain::set_heading_source, this, std::placeholders::_1, std::placeholders::_2));
      m_get_heading_source_service = this->create_service<ros2_mscl::srv::GetHeadingSource>("get_heading_source", std::bind(&Microstrain::get_heading_source, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_set_accel_noise_service = this->create_service<ros2_mscl::srv::SetAccelNoise>("set_accel_noise", std::bind(&Microstrain::set_accel_noise, this, std::placeholders::_1, std::placeholders::_2));
      m_get_accel_noise_service = this->create_service<ros2_mscl::srv::GetAccelNoise>("get_accel_noise", std::bind(&Microstrain::get_accel_noise, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_set_gyro_noise_service = this->create_service<ros2_mscl::srv::SetGyroNoise>("set_gyro_noise", std::bind(&Microstrain::set_gyro_noise, this, std::placeholders::_1, std::placeholders::_2));
      m_get_gyro_noise_service = this->create_service<ros2_mscl::srv::GetGyroNoise>("get_gyro_noise", std::bind(&Microstrain::get_gyro_noise, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_set_mag_noise_service = this->create_service<ros2_mscl::srv::SetMagNoise>("set_mag_noise", std::bind(&Microstrain::set_mag_noise, this, std::placeholders::_1, std::placeholders::_2));
      m_get_mag_noise_service = this->create_service<ros2_mscl::srv::GetMagNoise>("get_mag_noise", std::bind(&Microstrain::get_mag_noise, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_set_accel_bias_model_service = this->create_service<ros2_mscl::srv::SetAccelBiasModel>("set_accel_bias_model", std::bind(&Microstrain::set_accel_bias_model, this, std::placeholders::_1, std::placeholders::_2));
      m_get_accel_bias_model_service = this->create_service<ros2_mscl::srv::GetAccelBiasModel>("get_accel_bias_model", std::bind(&Microstrain::get_accel_bias_model, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_set_gyro_bias_model_service = this->create_service<ros2_mscl::srv::SetGyroBiasModel>("set_gyro_bias_model", std::bind(&Microstrain::set_gyro_bias_model, this, std::placeholders::_1, std::placeholders::_2));
      m_get_gyro_bias_model_service = this->create_service<ros2_mscl::srv::GetGyroBiasModel>("get_gyro_bias_model", std::bind(&Microstrain::get_gyro_bias_model, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_set_mag_adaptive_vals_service = this->create_service<ros2_mscl::srv::SetMagAdaptiveVals>("set_mag_adaptive_vals", std::bind(&Microstrain::set_mag_adaptive_vals, this, std::placeholders::_1, std::placeholders::_2));
      m_get_mag_adaptive_vals_service = this->create_service<ros2_mscl::srv::GetMagAdaptiveVals>("get_mag_adaptive_vals", std::bind(&Microstrain::get_mag_adaptive_vals, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_set_mag_dip_adaptive_vals_service = this->create_service<ros2_mscl::srv::SetMagDipAdaptiveVals>("set_mag_dip_adaptive_vals", std::bind(&Microstrain::set_mag_dip_adaptive_vals, this, std::placeholders::_1, std::placeholders::_2));
      m_get_mag_dip_adaptive_vals_service = this->create_service<ros2_mscl::srv::GetMagDipAdaptiveVals>("get_mag_dip_adaptive_vals", std::bind(&Microstrain::get_mag_dip_adaptive_vals, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_set_gravity_adaptive_vals_service = this->create_service<ros2_mscl::srv::SetGravityAdaptiveVals>("set_gravity_adaptive_vals", std::bind(&Microstrain::set_gravity_adaptive_vals, this, std::placeholders::_1, std::placeholders::_2));
      m_get_gravity_adaptive_vals_service = this->create_service<ros2_mscl::srv::GetGravityAdaptiveVals>("get_gravity_adaptive_vals", std::bind(&Microstrain::get_gravity_adaptive_vals, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_set_zero_angle_update_threshold_service = this->create_service<ros2_mscl::srv::SetZeroAngleUpdateThreshold>("set_zero_angle_update_threshold", std::bind(&Microstrain::set_zero_angle_update_threshold, this, std::placeholders::_1, std::placeholders::_2));
      m_get_zero_angle_update_threshold_service = this->create_service<ros2_mscl::srv::GetZeroAngleUpdateThreshold>("get_zero_angle_update_threshold", std::bind(&Microstrain::get_zero_angle_update_threshold, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_set_zero_velocity_update_threshold_service = this->create_service<ros2_mscl::srv::SetZeroVelocityUpdateThreshold>("set_zero_velocity_update_threshold", std::bind(&Microstrain::set_zero_velocity_update_threshold, this, std::placeholders::_1, std::placeholders::_2));
      m_get_zero_velocity_update_threshold_service = this->create_service<ros2_mscl::srv::GetZeroVelocityUpdateThreshold>("get_zero_velocity_update_threshold", std::bind(&Microstrain::get_zero_velocity_update_threshold, this, std::placeholders::_1, std::placeholders::_2));
    }

    //Reference position service
    if (m_inertial_device->features().supportsCommand(mscl::MipTypes::Command::CMD_EF_SET_REF_POSITION))
    {
      m_set_reference_position_service = this->create_service<ros2_mscl::srv::SetReferencePosition>("set_reference_position", std::bind(&Microstrain::set_reference_position, this, std::placeholders::_1, std::placeholders::_2));
      m_get_reference_position_service = this->create_service<ros2_mscl::srv::GetReferencePosition>("get_reference_position", std::bind(&Microstrain::get_reference_position, this, std::placeholders::_1, std::placeholders::_2));
    }

    //Dynamics mode service
    if (m_inertial_device->features().supportsCommand(mscl::MipTypes::Command::CMD_EF_VEHIC_DYNAMICS_MODE))
    {
      m_set_dynamics_mode_service = this->create_service<ros2_mscl::srv::SetDynamicsMode>("set_dynamics_mode", std::bind(&Microstrain::set_dynamics_mode, this, std::placeholders::_1, std::placeholders::_2));
      m_get_dynamics_mode_service = this->create_service<ros2_mscl::srv::GetDynamicsMode>("get_dynamics_mode", std::bind(&Microstrain::get_dynamics_mode, this, std::placeholders::_1, std::placeholders::_2));
    } 
 
    //Device Settings Service
    if (m_inertial_device->features().supportsCommand(mscl::MipTypes::Command::CMD_SAVE_STARTUP_SETTINGS))
    {
      m_device_settings_service = this->create_service<ros2_mscl::srv::DeviceSettings>("device_settings", std::bind(&Microstrain::device_settings, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_external_heading_service = this->create_service<ros2_mscl::srv::ExternalHeadingUpdate>("external_heading", std::bind(&Microstrain::external_heading_update, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    {
      m_set_relative_position_reference_service = this->create_service<ros2_mscl::srv::SetRelativePositionReference>("set_relative_position_reference", std::bind(&Microstrain::set_relative_position_reference, this, std::placeholders::_1, std::placeholders::_2));
      m_get_relative_position_reference_service = this->create_service<ros2_mscl::srv::GetRelativePositionReference>("get_relative_position_reference", std::bind(&Microstrain::get_relative_position_reference, this, std::placeholders::_1, std::placeholders::_2));
    }
    ...
    double max_rate = std::max({m_publish_imu            ? m_imu_data_rate : 1,
    ...
    m_timer_update_rate_hz = std::min(2 * max_rate, 1000.0);
...
bool Microstrain::activate_node()
{
  ...
  m_main_loop_timer     = this->create_wall_timer(timer_interval_ms, std::bind(&Microstrain::parsing_timer_callback, this));
  m_device_status_timer = this->create_wall_timer(std::chrono::seconds(1), std::bind(&Microstrain::device_status_callback, this));
  ...
  //IMU Publishers
  if(m_imu_pub)
   m_imu_pub->on_activate();
   
  if(m_mag_pub)
   m_mag_pub->on_activate();

  if(m_gps_corr_pub)
   m_gps_corr_pub->on_activate();
  ...
  {
    if(m_gnss_pub[i])
     m_gnss_pub[i]->on_activate();
  
    if(m_gnss_odom_pub[i])
     m_gnss_odom_pub[i]->on_activate();

    if(m_gnss_time_pub[i])
     m_gnss_time_pub[i]->on_activate();

    if(m_gnss_aiding_status_pub[i])
     m_gnss_aiding_status_pub[i]->on_activate();
  }

  //RTK Data publisher
  if(m_rtk_pub)
   m_rtk_pub->on_activate();
  
  //Filter Publishers
  if(m_filter_status_pub)
   m_filter_status_pub->on_activate();

  if(m_filter_heading_pub)
   m_filter_heading_pub->on_activate();

  if(m_filter_heading_state_pub)
   m_filter_heading_state_pub->on_activate();

  if(m_filter_pub)
   m_filter_pub->on_activate();

  if(m_filtered_imu_pub)
   m_filtered_imu_pub->on_activate();

  if(m_filter_relative_pos_pub)
   m_filter_relative_pos_pub->on_activate();
...
bool Microstrain::deactivate_node()
{
  ...
  //IMU Publishers
  if(m_imu_pub)
   m_imu_pub->on_deactivate();
   
  if(m_mag_pub)
   m_mag_pub->on_deactivate();

  if(m_gps_corr_pub)
   m_gps_corr_pub->on_deactivate();
  ...
  {
    if(m_gnss_pub[i])
     m_gnss_pub[i]->on_deactivate();
  
    if(m_gnss_odom_pub[i])
     m_gnss_odom_pub[i]->on_deactivate();

    if(m_gnss_time_pub[i])
     m_gnss_time_pub[i]->on_deactivate();

    if(m_gnss_aiding_status_pub[i])
     m_gnss_aiding_status_pub[i]->on_deactivate();
  }

  //RTK Data publisher
  if(m_rtk_pub)
   m_rtk_pub->on_deactivate();
  
  //Filter Publishers
  if(m_filter_status_pub)
   m_filter_status_pub->on_deactivate();

  if(m_filter_heading_pub)
   m_filter_heading_pub->on_deactivate();

  if(m_filter_heading_state_pub)
   m_filter_heading_state_pub->on_deactivate();

  if(m_filter_pub)
   m_filter_pub->on_deactivate();

  if(m_filtered_imu_pub)
   m_filtered_imu_pub->on_deactivate();

  if(m_filter_relative_pos_pub)
   m_filter_relative_pos_pub->on_deactivate();
...
bool Microstrain::shutdown_or_cleanup_node()
{
 ...
 //IMU Publishers
  ...
  for(int i=0; i< NUM_GNSS; i++)
  {
    ...
    if(m_gnss_odom_pub[i])
     m_gnss_odom_pub[i].reset();

    if(m_gnss_time_pub[i])
     m_gnss_time_pub[i].reset();

    if(m_gnss_aiding_status_pub[i])
     m_gnss_aiding_status_pub[i].reset();
  }
  ...
  //Filter Publishers
  if(m_filter_status_pub)
   m_filter_status_pub.reset();

  if(m_filter_heading_pub)
   m_filter_heading_pub.reset();

  if(m_filter_heading_state_pub)
   m_filter_heading_state_pub.reset();

  if(m_filter_pub)
   m_filter_pub.reset();

  if(m_filtered_imu_pub)
   m_filtered_imu_pub.reset();

  if(m_filter_relative_pos_pub)
   m_filter_relative_pos_pub.reset();
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// Single MIP Packet Parsing Function
/////////////////////////////////////////////////////////////////////////////////////////////////////

void Microstrain::parse_single_mip_packet(const mscl::MipDataPacket &packet)
{
...
void Microstrain::parse_imu_packet(const mscl::MipDataPacket &packet)
{
  ...
  //IMU timestamp
  m_imu_msg.header.stamp    = rclcpp::Time(static_cast<int64_t>(time));
  ...
  //Publish
  if(m_publish_gps_corr && m_gps_corr_pub->is_activated())
    m_gps_corr_pub->publish(m_gps_corr_msg);

  if(m_publish_imu && m_imu_pub->is_activated())
  {
    m_imu_pub->publish(m_imu_msg);
  
    if(has_mag && m_mag_pub->is_activated())
...
void Microstrain::parse_filter_packet(const mscl::MipDataPacket &packet)
{
  ...
  m_filtered_imu_msg.header.stamp    = rclcpp::Time(static_cast<int64_t>(time));
  m_filtered_imu_msg.header.frame_id = m_filter_frame_id;
  
  //Nav odom timestamp and frame
  m_filter_msg.header.stamp    = rclcpp::Time(static_cast<int64_t>(time));
  ...
  m_filter_relative_pos_msg.header.stamp    = rclcpp::Time(static_cast<int64_t>(time));
  ...
  for(mscl::MipDataPoint point : points)
  {
    ...
    case mscl::MipTypes::CH_FIELD_ESTFILTER_POSITION_AIDING_STATUS:
    {
      if(point.hasAddlIdentifiers())
      {
        ...
        {
          RCLCPP_INFO(this->get_logger(), "Point Qualifier %d", (int)point.qualifier());
        }
      }
    }break;
  ...
  if(m_publish_filter)
  {
    if(m_filtered_imu_pub->is_activated())
      m_filtered_imu_pub->publish(m_filtered_imu_msg);

    if(m_filter_pub->is_activated())
      m_filter_pub->publish(m_filter_msg);

    if(m_filter_status_pub->is_activated())
      m_filter_status_pub->publish(m_filter_status_msg);

    if(m_filter_heading_pub->is_activated())
      m_filter_heading_pub->publish(m_filter_heading_msg);

    if(m_filter_heading_state_pub->is_activated())
      m_filter_heading_state_pub->publish(m_filter_heading_state_msg);
  }
  
  if(m_publish_filter_relative_pos && m_filter_relative_pos_pub->is_activated())
    m_filter_relative_pos_pub->publish(m_filter_relative_pos_msg); 

  for(i=0; i<NUM_GNSS; i++)
  {
    if(m_publish_gnss_aiding_status[i] && gnss_aiding_status_received[i] && m_gnss_aiding_status_pub[i]->is_activated())
...
void Microstrain::parse_gnss_packet(const mscl::MipDataPacket &packet, int gnss_id)
{
  ...
  //GPS Fix time
  m_gnss_msg[gnss_id].header.stamp    = rclcpp::Time(static_cast<int64_t>(time));
  m_gnss_msg[gnss_id].header.frame_id = m_gnss_frame_id[gnss_id];
  
  //GPS Odom time
  m_gnss_odom_msg[gnss_id].header.stamp    = rclcpp::Time(static_cast<int64_t>(time));
  ...
  //GPS Time reference
  m_gnss_time_msg[gnss_id].header.stamp    = this->get_clock()->now();
  m_gnss_time_msg[gnss_id].header.frame_id = m_gnss_frame_id[gnss_id];
  m_gnss_time_msg[gnss_id].time_ref        = rclcpp::Time(static_cast<int64_t>(time));
  ...
  {
    if(m_gnss_pub[gnss_id]->is_activated())
      m_gnss_pub[gnss_id]->publish(m_gnss_msg[gnss_id]);
    
    if(m_gnss_odom_pub[gnss_id]->is_activated())
      m_gnss_odom_pub[gnss_id]->publish(m_gnss_odom_msg[gnss_id]);

    if(time_valid && m_gnss_time_pub[gnss_id]->is_activated())
...
void Microstrain::parse_rtk_packet(const mscl::MipDataPacket& packet)
{
  ...
  //Publish
  if(m_publish_rtk && m_rtk_pub->is_activated())
...
void Microstrain::vel_zupt() 
{
  ...
  rclcpp::AsyncSpinner spinner(4);

  while(rclcpp::ok() && m_vel_still) 
  {
...
void Microstrain::external_gps_time_callback(const sensor_msgs::TimeReference& time)
{
  if(m_inertial_device)
  {
    try
    {
      ...
      RCLCPP_INFO(this->get_logger(), "GPS Update: w%i, s%ld", weeks, secs);
    }
    catch(mscl::Error &e)
    {
      RCLCPP_ERROR(this->get_logger(), "Error: %s", e.what());
...
void Microstrain::reset_filter(const std::shared_ptr<std_srvs::srv::Empty::Request> req, 
...
void Microstrain::set_accel_bias(const std::shared_ptr<ros2_mscl::srv::SetAccelBias::Request> req, 
                                 std::shared_ptr<ros2_mscl::srv::SetAccelBias::Response> res)
{
  res->success = false;
...
void Microstrain::get_accel_bias(const std::shared_ptr<ros2_mscl::srv::GetAccelBias::Request> req, 
                                 std::shared_ptr<ros2_mscl::srv::GetAccelBias::Response> res)
{
  res->success = false;
...
void Microstrain::set_gyro_bias(const std::shared_ptr<ros2_mscl::srv::SetGyroBias::Request> req, 
                                std::shared_ptr<ros2_mscl::srv::SetGyroBias::Response> res)
{
  res->success = false;
...
void Microstrain::get_gyro_bias(const std::shared_ptr<ros2_mscl::srv::GetGyroBias::Request> req, 
                                std::shared_ptr<ros2_mscl::srv::GetGyroBias::Response> res)
{
  res->success = false;
...
void Microstrain::gyro_bias_capture(const std::shared_ptr<std_srvs::srv::Trigger::Request> req, 
...
void Microstrain::set_hard_iron_values(const std::shared_ptr<ros2_mscl::srv::SetHardIronValues::Request> req, 
                                       std::shared_ptr<ros2_mscl::srv::SetHardIronValues::Response> res)
{
  res->success = false;
...
void Microstrain::get_hard_iron_values(const std::shared_ptr<ros2_mscl::srv::GetHardIronValues::Request> req, 
                                       std::shared_ptr<ros2_mscl::srv::GetHardIronValues::Response> res)
{
  res->success = false;
  RCLCPP_INFO(this->get_logger(), "Getting gyro bias values");
...
void Microstrain::set_soft_iron_matrix(const std::shared_ptr<ros2_mscl::srv::SetSoftIronMatrix::Request> req, 
                                       std::shared_ptr<ros2_mscl::srv::SetSoftIronMatrix::Response> res)
{
  res->success = false;
...
void Microstrain::get_soft_iron_matrix(const std::shared_ptr<ros2_mscl::srv::GetSoftIronMatrix::Request> req, 
                                       std::shared_ptr<ros2_mscl::srv::GetSoftIronMatrix::Response> res)
{
  res->success = false;
...
void Microstrain::set_complementary_filter(const std::shared_ptr<ros2_mscl::srv::SetComplementaryFilter::Request> req, 
                                           std::shared_ptr<ros2_mscl::srv::SetComplementaryFilter::Response> res)
{
...
void Microstrain::get_complementary_filter(const std::shared_ptr<ros2_mscl::srv::GetComplementaryFilter::Request> req, 
                                           std::shared_ptr<ros2_mscl::srv::GetComplementaryFilter::Response> res)
{
  res->success = false;
...
void Microstrain::set_heading_source(const std::shared_ptr<ros2_mscl::srv::SetHeadingSource::Request> req, 
                                     std::shared_ptr<ros2_mscl::srv::SetHeadingSource::Response> res)
{
  res->success = false;
...
void Microstrain::get_heading_source(const std::shared_ptr<ros2_mscl::srv::GetHeadingSource::Request> req, 
                                     std::shared_ptr<ros2_mscl::srv::GetHeadingSource::Response> res)
{
  res->success = false;
...
void Microstrain::set_sensor2vehicle_rotation(const std::shared_ptr<ros2_mscl::srv::SetSensor2VehicleRotation::Request> req, 
                                              std::shared_ptr<ros2_mscl::srv::SetSensor2VehicleRotation::Response> res)
{
  res->success = false;
...
void Microstrain::get_sensor2vehicle_rotation(const std::shared_ptr<ros2_mscl::srv::GetSensor2VehicleRotation::Request> req, 
                                              std::shared_ptr<ros2_mscl::srv::GetSensor2VehicleRotation::Response> res)
{
  res->success = false;
...
void Microstrain::set_sensor2vehicle_offset(const std::shared_ptr<ros2_mscl::srv::SetSensor2VehicleOffset::Request> req, 
                                            std::shared_ptr<ros2_mscl::srv::SetSensor2VehicleOffset::Response> res)
{
  res->success = false;
...
void Microstrain::get_sensor2vehicle_offset(const std::shared_ptr<ros2_mscl::srv::GetSensor2VehicleOffset::Request> req, 
                                            std::shared_ptr<ros2_mscl::srv::GetSensor2VehicleOffset::Response> res)
{
  res->success = false;
...
void Microstrain::get_sensor2vehicle_transformation(const std::shared_ptr<ros2_mscl::srv::GetSensor2VehicleTransformation::Request> req, 
                                                    std::shared_ptr<ros2_mscl::srv::GetSensor2VehicleTransformation::Response> res)
{
  res->success = false;
...
void Microstrain::set_reference_position(const std::shared_ptr<ros2_mscl::srv::SetReferencePosition::Request> req, 
                                         std::shared_ptr<ros2_mscl::srv::SetReferencePosition::Response> res)
{
  res->success = false;
...
void Microstrain::get_reference_position(const std::shared_ptr<ros2_mscl::srv::GetReferencePosition::Request> req, 
                                         std::shared_ptr<ros2_mscl::srv::GetReferencePosition::Response> res)
{
  res->success = false;
...
void Microstrain::set_coning_sculling_comp(const std::shared_ptr<ros2_mscl::srv::SetConingScullingComp::Request> req, 
                                           std::shared_ptr<ros2_mscl::srv::SetConingScullingComp::Response> res)
{
  res->success = false;
...
void Microstrain::get_coning_sculling_comp(const std::shared_ptr<ros2_mscl::srv::GetConingScullingComp::Request> req, 
                                           std::shared_ptr<ros2_mscl::srv::GetConingScullingComp::Response> res)
{
  res->success = false;
...
void Microstrain::set_estimation_control_flags(const std::shared_ptr<ros2_mscl::srv::SetEstimationControlFlags::Request> req, 
                                               std::shared_ptr<ros2_mscl::srv::SetEstimationControlFlags::Response> res)
{
  res->success = false;
...
void Microstrain::get_estimation_control_flags(const std::shared_ptr<ros2_mscl::srv::GetEstimationControlFlags::Request> req, 
                                               std::shared_ptr<ros2_mscl::srv::GetEstimationControlFlags::Response> res)
{
  res->success = false;
...
void Microstrain::set_zero_angle_update_threshold(const std::shared_ptr<ros2_mscl::srv::SetZeroAngleUpdateThreshold::Request> req, 
                                                  std::shared_ptr<ros2_mscl::srv::SetZeroAngleUpdateThreshold::Response> res)
{
  res->success = false;
...
void Microstrain::get_zero_angle_update_threshold(const std::shared_ptr<ros2_mscl::srv::GetZeroAngleUpdateThreshold::Request> req, 
                                                  std::shared_ptr<ros2_mscl::srv::GetZeroAngleUpdateThreshold::Response> res)
{
  res->success = false;
...
void Microstrain::set_zero_velocity_update_threshold(const std::shared_ptr<ros2_mscl::srv::SetZeroVelocityUpdateThreshold::Request> req, 
                                                     std::shared_ptr<ros2_mscl::srv::SetZeroVelocityUpdateThreshold::Response> res)
{
  res->success = false;
...
void Microstrain::get_zero_velocity_update_threshold(const std::shared_ptr<ros2_mscl::srv::GetZeroVelocityUpdateThreshold::Request> req, 
                                                     std::shared_ptr<ros2_mscl::srv::GetZeroVelocityUpdateThreshold::Response> res)
{
  res->success = false;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
void Microstrain::set_tare_orientation(const std::shared_ptr<ros2_mscl::srv::SetTareOrientation::Request> req, 
                                       std::shared_ptr<ros2_mscl::srv::SetTareOrientation::Response> res)
{
  res->success = false;
...
void Microstrain::set_accel_noise(const std::shared_ptr<ros2_mscl::srv::SetAccelNoise::Request> req, 
                                  std::shared_ptr<ros2_mscl::srv::SetAccelNoise::Response> res)
{
  res->success = false;
...
void Microstrain::get_accel_noise(const std::shared_ptr<ros2_mscl::srv::GetAccelNoise::Request> req, 
                                  std::shared_ptr<ros2_mscl::srv::GetAccelNoise::Response> res)
{
  res->success = false;
...
void Microstrain::set_gyro_noise(const std::shared_ptr<ros2_mscl::srv::SetGyroNoise::Request> req, 
                                 std::shared_ptr<ros2_mscl::srv::SetGyroNoise::Response> res)
{
  res->success = false;
...
void Microstrain::get_gyro_noise(const std::shared_ptr<ros2_mscl::srv::GetGyroNoise::Request> req, 
                                 std::shared_ptr<ros2_mscl::srv::GetGyroNoise::Response> res)
{
  res->success = false;
...
void Microstrain::set_mag_noise(const std::shared_ptr<ros2_mscl::srv::SetMagNoise::Request> req, 
                                std::shared_ptr<ros2_mscl::srv::SetMagNoise::Response> res)
{
  res->success = false;
...
void Microstrain::get_mag_noise(const std::shared_ptr<ros2_mscl::srv::GetMagNoise::Request> req, 
                                std::shared_ptr<ros2_mscl::srv::GetMagNoise::Response> res)
{
  res->success = false;
...
void Microstrain::set_gyro_bias_model(const std::shared_ptr<ros2_mscl::srv::SetGyroBiasModel::Request> req, 
                                      std::shared_ptr<ros2_mscl::srv::SetGyroBiasModel::Response> res)
{
  res->success = false;
...
void Microstrain::get_gyro_bias_model(const std::shared_ptr<ros2_mscl::srv::GetGyroBiasModel::Request> req, 
                                      std::shared_ptr<ros2_mscl::srv::GetGyroBiasModel::Response> res)
{
  res->success = false;
...
void Microstrain::set_accel_bias_model(const std::shared_ptr<ros2_mscl::srv::SetAccelBiasModel::Request> req, 
                                       std::shared_ptr<ros2_mscl::srv::SetAccelBiasModel::Response> res)
{
  res->success = false;
...
void Microstrain::get_accel_bias_model(const std::shared_ptr<ros2_mscl::srv::GetAccelBiasModel::Request> req, 
                                       std::shared_ptr<ros2_mscl::srv::GetAccelBiasModel::Response> res)
{
  res->success = false;
...
void Microstrain::set_gravity_adaptive_vals(const std::shared_ptr<ros2_mscl::srv::SetGravityAdaptiveVals::Request> req, 
                                            std::shared_ptr<ros2_mscl::srv::SetGravityAdaptiveVals::Response> res)
{
  res->success = false;
...
void Microstrain::get_gravity_adaptive_vals(const std::shared_ptr<ros2_mscl::srv::GetGravityAdaptiveVals::Request> req, 
                                            std::shared_ptr<ros2_mscl::srv::GetGravityAdaptiveVals::Response> res)
{
  res->success = false;
...
void Microstrain::set_mag_adaptive_vals(const std::shared_ptr<ros2_mscl::srv::SetMagAdaptiveVals::Request> req, 
                                        std::shared_ptr<ros2_mscl::srv::SetMagAdaptiveVals::Response> res)
{
  res->success = false;
...
void Microstrain::get_mag_adaptive_vals(const std::shared_ptr<ros2_mscl::srv::GetMagAdaptiveVals::Request> req, 
                                        std::shared_ptr<ros2_mscl::srv::GetMagAdaptiveVals::Response> res)
{
  res->success = false;
  ...
  if(m_inertial_device)
  {
    try
    {
      ...
      RCLCPP_INFO(this->get_logger(), "Mag magnitude error adaptive measurement values are: Enable: %i, Parameters: %f %f %f %f %f %f",
...
void Microstrain::set_mag_dip_adaptive_vals(const std::shared_ptr<ros2_mscl::srv::SetMagDipAdaptiveVals::Request> req, 
                                            std::shared_ptr<ros2_mscl::srv::SetMagDipAdaptiveVals::Response> res)
{
  res->success = false;
...
void Microstrain::get_mag_dip_adaptive_vals(const std::shared_ptr<ros2_mscl::srv::GetMagDipAdaptiveVals::Request> req, 
                                            std::shared_ptr<ros2_mscl::srv::GetMagDipAdaptiveVals::Response> res)
{
  res->success = false;
...
void Microstrain::set_dynamics_mode(const std::shared_ptr<ros2_mscl::srv::SetDynamicsMode::Request> req, 
                                    std::shared_ptr<ros2_mscl::srv::SetDynamicsMode::Response> res)
{
  res->success = false;
...
void Microstrain::get_dynamics_mode(const std::shared_ptr<ros2_mscl::srv::GetDynamicsMode::Request> req, 
                                    std::shared_ptr<ros2_mscl::srv::GetDynamicsMode::Response> res)
{
  res->success = false;
...
void Microstrain::device_settings(const std::shared_ptr<ros2_mscl::srv::DeviceSettings::Request> req, 
                                  std::shared_ptr<ros2_mscl::srv::DeviceSettings::Response> res)
{
  res->success = false;
...
void Microstrain::commanded_vel_zupt(const std::shared_ptr<std_srvs::srv::Trigger::Request> req, 
                                     std::shared_ptr<std_srvs::srv::Trigger::Response> res)
{
  res->success = false;
...
void Microstrain::commanded_ang_rate_zupt(const std::shared_ptr<std_srvs::srv::Trigger::Request> req, 
                                          std::shared_ptr<std_srvs::srv::Trigger::Response> res)
{
  res->success = false;
...
void Microstrain::external_heading_update(const std::shared_ptr<ros2_mscl::srv::ExternalHeadingUpdate::Request> req, 
                                          std::shared_ptr<ros2_mscl::srv::ExternalHeadingUpdate::Response> res)
{
  res->success = false;
...
void Microstrain::print_packet_stats()
{
  ...
  if(m_inertial_device->features().supportsCommand(mscl::MipTypes::Command::CMD_DEVICE_STATUS))
  {
    if(m_inertial_device->features().supportedStatusSelectors().size() > 1) 
    {
      ...
      RCLCPP_DEBUG(this->get_logger(), "%u IMU (%u errors) Packets",
      ...
      RCLCPP_DEBUG(this->get_logger(), "%u FILTER (%u errors)    %u IMU (%u errors)    %u GPS (%u errors) Packets",
      ...
      RCLCPP_DEBUG(this->get_logger(), "%u FILTER (%u errors)    %u IMU (%u errors) Packets",
                         m_filter_valid_packet_count, m_filter_timeout_packet_count,
                         m_imu_valid_packet_count,    m_imu_timeout_packet_count + m_imu_checksum_error_packet_count);
    }
  }
